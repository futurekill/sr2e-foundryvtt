...
- [ ] **Automatic Combat Turn countdown (0.97.0):** put the mage in a combat and start it, then hand a sustained spell to an elemental. The next "Next Round" is free, because it took the spell over mid-turn; each round after that costs it 1 Force, whispered as "N Combat Turns left". At 0 the spell ends BEFORE the new Combat Turn's initiative. Editing the round number charges nothing. A mage not in the combat is not charged.
- [ ] **Recovery:** if a turn couldn't be counted, a whispered card "Count this Combat Turn" appears after the new round starts. Clicking it counts once; after the next round has ended the card expires, and the fix is "−1 Combat Turn" (or "Starting turn already passed" for the free turn).
...
- [ ] **Learning a spell (0.98.0, p.132–133):** the Magic tab's "Learn" button opens a dialog with the spell, Force, library/lodge rating, teacher successes and Aid Study. The readout shows the dice (Sorcery + Magical Theory), TN = 2 × Force, the Karma cost and the days. Roll → a learning card; "Complete learning" adds the spell (clean, not sustaining), charges Force in Karma and reports days rounded up. Zero successes: nothing added, no Karma, Force days wasted. A Karma reroll before completing updates the card; afterwards the test's Karma buttons are gone.
- [ ] **Aid Study (p.141):** a bound elemental of the spell's category adds its Force in dice for 1 service, and its Force is unchanged. Choosing it again for the same spell is refused.
- [ ] **Spell Defense aid (p.141):** "Allocate" offers a fire elemental. Reserving 3 dice costs 1 service (if idle) but no Force. Resisting a combat spell adds those dice and then lowers its Force. Clear, refresh or the Magic Pool reset drops the reservation free. Reserved dice aren't offered when casting.
